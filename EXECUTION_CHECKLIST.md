...
| Done | ID | Task | Owner | Acceptance Test |
|---|---|---|---|---|
| [ ] | W2-01 | Implement JWT verification middleware with supported alg list | BE + SEC | Valid token passes; expired/invalid token returns 401 |
| [ ] | W2-02 | Implement keyset-based validation (`kid`) | BE + SEC | Tokens signed by old and new active keys both validate during rotation window |
| [ ] | W2-03 | Implement optional JWKS fetch + cache TTL | BE | JWKS-backed token validates; stale cache refreshes on TTL expiry |
| [ ] | W2-04 | Implement tenant resolvers (subdomain/header/path) | BE | Unit tests cover valid and invalid cases for all 3 strategies |
| [ ] | W2-05 | Enforce tenant match (`resolved tenant == JWT tenant_id`) | BE + SEC | Mismatch returns 403 before proxying |
| [ ] | W2-06 | Strip spoofed identity headers and re-inject canonical headers | BE + SEC | Client `X-User-ID`/`X-Tenant-ID` values never reach upstream unchanged |
| [ ] | W2-07 | Add trusted proxy handling for `X-Forwarded-For`/`X-Real-IP` | BE + SEC | Untrusted hop cannot spoof client IP; trusted hop can forward expected value |
